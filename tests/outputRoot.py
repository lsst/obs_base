...
class OutputRootTestCase(unittest.TestCase):
    ...
    def testBackup(self):
        mapper1 = MinMapper1(outputRoot="testOutput")
        butler1 = dafPersist.Butler(outputs=dafPersist.Repository.cfg(
            mode='w',
            storageCfg=dafPersist.PosixStorage.cfg(root="testOutput"),
            mapper=mapper1)
        )
        ...
        butler2 = dafPersist.Butler(
            ...
            outputs=dafPersist.Repository.cfg(
                mode='rw',
                storageCfg=dafPersist.PosixStorage.cfg(root="testOutput2"),
                mapper=mapper2),
        )
        b3 = afwGeom.Box2I(b2)
        b3.grow(1)
