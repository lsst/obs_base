...
class TestCamera(cameraGeom.Camera):
    ...
    * ``visit``: exposure number; test data includes one exposure
    ...
    def __new__(cls):
        ...
        # from focal plane (in mm) to field angle (in radians). Thus the
        # coefficients are:
        # C0: always 0, for continuity at the center of the focal plane;
        #     units are rad
    ...
    def _makeDetectorConfigList(cls):
        ...
        """
        # this camera has one detector that corresponds to a subregion of
        # lsstSim detector R:2,2 S:0,0
    ...
    def _makeAmplifierCatalog(cls):
        ...
        of usable bias region (which is used to set rawHOverscanBbox,
        despite the name), followed by the data.
        There is no other underscan or overscan.
        """
        ...
        for ampX in (0, 1):
            for ampY in (0, 1):
                # amplifier gain (e-/ADU) and read noiuse (ADU/pixel) from
                # lsstSim raw data note that obs_test amp <ampX><ampY> =
                # lsstSim amp C<ampY>,<ampX> (axes are swapped)
                gain = {
                    (0, 0): 1.7741,  # C0,0
                    (0, 1): 1.65881,  # C1,0
                    (1, 0): 1.74151,  # C0,1
